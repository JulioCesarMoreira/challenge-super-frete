...
describe('Cálcular frete usando conjunto de dados pré definido e validando o correto cálculo e estabilidade da funcionalidade', () => {
  ...
  it('Cálculo de frete com as opções de recebimento em mão e seguro (Mão Própria + Seguro)', () => {
    ...
    const withAcknowledgmentOfReceiptAndValueDeclarationAdditional: FormFields = {
      ...
      additionalServices: ["valueDeclaration", "selfHand"],
    ...
    // Result validation
    cy.validatingFormResult(fieldToValidEnum.MAIN_PAC, ['32,64']);
    cy.validatingFormResult(fieldToValidEnum.PAC, ['20,57']);
    cy.validatingFormResult(fieldToValidEnum.MAIN_SEDEX, ['32,64']);
    cy.validatingFormResult(fieldToValidEnum.SEDEX, ['13,24']);
