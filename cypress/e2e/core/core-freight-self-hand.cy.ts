...
describe('Cálcular frete usando conjunto de dados pré definido e validando o correto cálculo e estabilidade da funcionalidade (Mão Própria)', () => {
  ...
  it('Calculo de frete com a opção Recebimento em mãos (Mão Própria)', () => {
    const withSelfHandAdditional: FormFields = {
      ...
      additionalServices: ["selfHand"],

      destinyZipCode: '05407-002', // Cep de Destino
    };

    cy.fillFreightCalcForm(withSelfHandAdditional);
    ...
    // Result validation
    cy.validatingFormResult(fieldToValidEnum.MAIN_PAC, ['31,15']);
    cy.validatingFormResult(fieldToValidEnum.PAC, ['19,73']);
    cy.validatingFormResult(fieldToValidEnum.MAIN_SEDEX, ['31,15']);
    cy.validatingFormResult(fieldToValidEnum.SEDEX, ['12,41']);
